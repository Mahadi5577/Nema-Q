...
% PDF outline: sections + subsections only. \paragraph heads under a
% \section otherwise jump bookmark levels and leak math tokens into
% PDF strings (hyperref warnings).
...
\begin{figure}[p]
\centering
\includegraphics[width=0.82\textwidth]{figs/cora_paired_frozen_vs_trained.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/citeseer_paired_frozen_vs_trained.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/pubmed_paired_frozen_vs_trained.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/disease_paired_frozen_vs_trained.pdf}
\caption{H5 paired-seed plots, one row per dataset (top to bottom: Cora, Citeseer, Pubmed, Disease). Left panel of each row: per-seed test accuracy of frozen-random PQC (NEMA-R) vs.\ trained PQC (NEMA-Q) connected by seed (green = frozen ahead). Right panel: the paired-difference distribution with its mean and Wilcoxon $p$. Slope direction, not vertical position, carries the H5 verdict.}
\label{fig:h5paired}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{figs/h1_geometry_delta.pdf}
\caption{Geometry $\times$ $\delta$: paired hyperbolic$-$Euclidean gain (mean $\pm$ 95\% CI) against dataset $\delta_{\text{mean}}$. The preregistered H1(b) prediction is a negative slope; the observed trend is positive. The in-panel pooled annotation is the registered statistic (Stouffer over one-sided Wilcoxon tests). Per-dataset paired-seed plots are in Fig.~\ref{fig:geopaired}.}
\label{fig:geometry}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.82\textwidth]{figs/cora_paired_geometry.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/citeseer_paired_geometry.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/pubmed_paired_geometry.pdf}\\[3pt]
\includegraphics[width=0.82\textwidth]{figs/disease_paired_geometry.pdf}
\caption{H1 per-dataset paired-seed plots, one row per dataset (top to bottom: Cora, Citeseer, Pubmed, Disease). Left panel of each row: per-seed test accuracy of trained hyperbolic NEMA-Q vs.\ its Euclidean control NEMA-E connected by seed. Right panel: the paired-difference distribution with its mean and Wilcoxon $p$.}
\label{fig:geopaired}
\end{figure}
...
Gradient saliency without faithfulness evidence is decoration; we therefore pair QOA with a three-check harness, run per dataset on the trained models:
\begin{enumerate}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{figs/cora_xai_02_qoa_faithfulness.pdf}\\[4pt]
\includegraphics[width=\textwidth]{figs/citeseer_xai_02_qoa_faithfulness.pdf}\\[4pt]
\includegraphics[width=\textwidth]{figs/pubmed_xai_02_qoa_faithfulness.pdf}\\[4pt]
\includegraphics[width=\textwidth]{figs/disease_xai_02_qoa_faithfulness.pdf}
\caption{QOA faithfulness harness, one row per dataset (top to bottom: Cora, Citeseer, Pubmed, Disease; pilot models). Each row's three panels are the masking logit-drops (top vs.\ bottom observable), perturbation flip rates (top vs.\ bottom), and the randomization Spearman $\rho$ against its $0.5$ threshold. Masking and perturbation faithfulness hold on all four datasets; the randomization check passes on the three citation networks and fails on Disease only.}
